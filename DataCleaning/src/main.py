...
def main():
    running_tasks = ecs.list_tasks(cluster="arn:aws:ecs:us-west-2:ACCOUNTNUMBERHERE:cluster/avi-to-mp4", desiredStatus="RUNNING")
    print("running tasks ", running_tasks)
    running_tasks = len(running_tasks['taskArns'])
    print("Starting!!")
    ...
    if running_tasks < 2:
        ...
        if not LOCAL_RUN:
            download_files_to_process(files_to_process)

        for file_name in files_to_process:
            print("FILE IS " + file_name)
            process_file(file_name)
            remove_original_avi_file(file_name)
            clear_temp_folder()
...
def get_files_to_process():
    ...
    avi_bucket = s3.Bucket(SOURCE_AVI_VIDEO_BUCKET)
    mp4_bucket = s3.Bucket(OUTPUT_MP4_VIDEO_BUCKET)

    avi_files = list(map(lambda x: x.key, avi_bucket.objects.all()))
    mp4_files = list(map(lambda x: (x.key.split("---")[0] + ".mp4").lower(), mp4_bucket.objects.all()))

    files_to_process = []
    for file in avi_files:
        mp4_file_name = file.lower().replace(".avi", ".mp4")
        if mp4_file_name not in mp4_files:
...
def download_files_to_process(files_to_process):
    for file_name in files_to_process:
        ...
        s3_client.download_file(Bucket=SOURCE_AVI_VIDEO_BUCKET,
                                Key=file_name,
...
Return: None
'''''''''
def upload_frames(file_name, num_clips):
    base_file_name = file_name.replace(".avi", "")

    for clip_num in range(num_clips):
        clip_postfix = f"---{clip_num:03d}"
        mp4_file_name = base_file_name + clip_postfix + ".mp4"

        cam = cv2.VideoCapture(TEMP_DIRECTORY + mp4_file_name)
        fps = cam.get(cv2.CAP_PROP_FPS)
        ...
        fight_num = get_max_fight_num(list(map(lambda x: x.key, s3.Bucket(OUTPUT_MP4_VIDEO_BUCKET).objects.all())))
        clip_postfix = f"---{clip_num:03d}"
        ...
        while True:
            ...
            if currentframe == target_frame:
                ...
                frame_name = mp4_file_name.replace(".mp4", "") + '_frame_' + str(currentframe) + '.jpg'
                cv2.imwrite(TEMP_DIRECTORY + frame_name, frame)
                target_frame += frame_increment

                upload_frame_name = "Fight_" + str(fight_num) + clip_postfix + '_frame_' + str(currentframe) + '.jpg'
                print("Uploading frame: " + upload_frame_name)
